...
const CampaignsPhrase = () => {
  ...
  const handleDeleteCampaign = async (campaignId) => {
    try {
      ...
      toast.success(i18n.t("campaignsPhrase.deleteSuccess"));
      getCampaigns();
    } catch (err) {
      toastError(err);
    }
  ...
  const renderWhatsAppConnections = (whatsappIds) => {
    if (!whatsappIds || !Array.isArray(whatsappIds) || whatsappIds.length === 0) {
      return <Typography variant="body2" color="textSecondary">{i18n.t("campaignsPhrase.noConnection")}</Typography>;
    }
    ...
    if (whatsappIds.length > 3) {
      ...
      return (
        <Box className={classes.connectionContainer}>
          ...
          <Tooltip title={`${i18n.t("campaignsPhrase.moreConnections")} ${remaining} ${i18n.t("campaignsPhrase.connection")}`}>
            <Chip 
              label={`+${remaining}`}
              className={classes.whatsappChip}
              size="small"
      ...
      <MainHeader>
        <Grid style={{ width: "99.6%" }} container>
          <Grid xs={12} sm={8} item>
            <Title>{i18n.t("campaignsPhrase.title")}</Title>
          </Grid>
              ...
              <Grid xs={6} sm={6} item>
                ...
                >
                  <Stack direction={"row"} gap={1}>
                    <AddCircle />
                    {i18n.t("campaignsPhrase.newCampaign")}
                  </Stack>
                </Button>
              </Grid>
            </Grid>
              ...
              {campaignflows && Array.isArray(campaignflows) && campaignflows.length > 0 ? (
                campaignflows.map((flow) => (
                  ...
                  >
                        ...
                        <Stack direction={"row"} alignItems="center">
                          ...
                          <Stack justifyContent={"center"} marginLeft={1}>
                            <Typography variant="body2" style={{ fontWeight: 500 }}>
                              {flow.name || i18n.t("campaignsPhrase.nameNotDefined")}
                            </Typography>
                            ...
                            {flow.phrase && Array.isArray(flow.phrase) && (
                              <Typography variant="caption" style={{ color: "#bbb" }}>
                                {flow.phrase.length} {i18n.t("campaignsPhrase.phrasesConfigured")}
                              </Typography>
                            )}
                          </Stack>
                        </Stack>
                    ...
                    <Grid item xs={3} align="center" style={{ color: "#ededed" }}>
                      <Stack justifyContent={"center"} height={"100%"}>
                        <Chip 
                          label={flow.status ? i18n.t("campaignsPhrase.active") : i18n.t("campaignsPhrase.disabled")}
                          size="small"
                    ...
                    <Grid item xs={3} align="end">
                      <Stack direction="row" justifyContent="flex-end" alignItems="center">
                        <IconButton
                          ...
                          title={i18n.t("campaignsPhrase.editCampaign")}
                        >
                        ...
                        <Can
                          ...
                          yes={() => (
                            <IconButton
                              ...
                              title={i18n.t("campaignsPhrase.deleteCampaign")}
                            >
                ...
                >
                  <Typography variant="body1">
                    {i18n.t("campaignsPhrase.noCampaignsFound")}
                  </Typography>
                  <Typography variant="body2" style={{ marginTop: "8px", color: "#bbb" }}>
                    {i18n.t("campaignsPhrase.clickToCreateFirst")}
                  </Typography>
                </Stack>
              )}
            </>

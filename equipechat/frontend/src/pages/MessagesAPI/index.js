...
const MessagesAPI = () => {
  ...
  const handleSendTextMessage = async (values) => {
    ...
    try {
      ...
      toast.success(i18n.t("messagesAPI.successMessage"));
    } catch (err) {
      toastError(err);
    }
  };
      ...
      <Grid container>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin} component="div">
            ...
            <b>{i18n.t("messagesAPI.API.media.endpoint")}</b> {getEndpoint()} <br />
            <b>{i18n.t("messagesAPI.API.media.method")}</b> POST <br />
            <b>{i18n.t("messagesAPI.API.media.headers")}</b> Authorization Bearer (token cadastrado) e
            Content-Type (multipart/form-data) <br />
            <b>{i18n.t("messagesAPI.API.media.formData")}</b> <br />
            <ul>
              <li>
              ...
              <li>
                <b>userId:</b> {i18n.t("messagesAPIInstructions.fieldUserId").replace('"userId": ', '').replace(/"/g, '')}
              </li>
              <li>
                <b>queueId:</b> {i18n.t("messagesAPIInstructions.fieldQueueId").replace('"queueId": ', '').replace(/"/g, '')}
              </li>
              <li>
                <b>medias: </b> {i18n.t("messagesAPI.API.media.file")}
              </li>
              <li>
                <b>sendSignature:</b>{" "}

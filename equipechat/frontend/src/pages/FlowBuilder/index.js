        ...
        <Stack direction="row" alignItems="flex-start" spacing={2}>
          ...
          <Box flex={1}>
            ...
            <div className={classes.flowActions}>
              <Chip
                size="small"
                label={flow.active ? i18n.t("flowBuilder.flowStatus.active") : i18n.t("flowBuilder.flowStatus.inactive")}
...
const FlowBuilder = () => {
  ...
  const handleDeleteWebhook = async (webhookId) => {
    try {
      ...
      toast.success(i18n.t("flowBuilder.toasts.flowDeleted"));
    } catch (err) {
      toastError(err);
    }
  };
  ...
  const handleDuplicateFlow = async (flowId) => {
    try {
      ...
      toast.success(i18n.t("flowBuilder.toasts.flowDuplicated"));
    } catch (err) {
      toastError(err);
    }
  };
  ...
  return (
    <MainContainer className={classes.mainContainer}>
      ...
      <ConfirmationModal
        title={
          deletingContact
            ? `${i18n.t("flowBuilder.confirmations.deleteTitle")} "${deletingContact.name}"?`
      ...
      >
        {deletingContact
          ? i18n.t("flowBuilder.confirmations.deleteMessage")
      ...
      <ConfirmationModal
        title={
          deletingContact
            ? `${i18n.t("flowBuilder.confirmations.duplicateTitle")} "${deletingContact.name}"?`
            : `${i18n.t("contacts.confirmationModal.importTitlte")}`
        }
      ...
      >
        {deletingContact
          ? i18n.t("flowBuilder.confirmations.duplicateMessage")
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>

      {/* Header seguindo padrão da página Tags */}
      <MainHeader>
        <Title>{i18n.t("flowBuilder.title")} ({webhooks.length})</Title>
        <MainHeaderButtonsWrapper>
          <TextField
            className={classes.searchField}
            placeholder={i18n.t("flowBuilder.searchPlaceholder")}
            type="search"
            value={searchParam}
        ...
        {loading && !webhooks.length ? (
          ...
          <div className={classes.emptyState}>
            ...
            <Typography className={classes.emptyTitle}>
              {searchParam ? i18n.t('flowBuilder.noFlowsFound') : i18n.t('flowBuilder.noFlowsCreated')}
            </Typography>
            <Typography className={classes.emptyDescription}>
              {searchParam 
                ? i18n.t('flowBuilder.tryOtherSearchTerms')
                : i18n.t('flowBuilder.createFirstFlowDescription')
              }
            </Typography>
            ...
            {!searchParam && (
              <Button
                ...
                style={{ marginTop: 24 }}
              >
                {i18n.t('flowBuilder.createFirstFlow')}
              </Button>
            )}
          </div>
        ) : (
      ...
      {isMobile && (
        <HideOnScroll>
          <Fab
            ...
            onClick={handleOpenContactModal}
            aria-label={i18n.t("flowBuilder.newFlow")}
          >
            <AddCircle />
          </Fab>
        </HideOnScroll>

...
const useStyles = makeStyles((theme) => ({
  ...
  languageItem: {
    ...
    border: `1px solid ${theme.mode === "light" ? "#e0e0e0" : "rgba(255,255,255,0.15)"}`,
    borderRadius: theme.spacing(0.5),
  ...
  languageCheckbox: {
    padding: theme.spacing(0.5),
    color: theme.mode === "light" ? grey[700] : "#E5E7EB",
    "&.Mui-checked": {
      color: theme.palette.primary.main,
    },
  },
  languageLabel: {
    fontSize: "0.875rem",
    fontWeight: 500,
    color: theme.mode === "light" ? theme.palette.text.primary : "#E5E7EB",
    [theme.breakpoints.down("sm")]: {
      fontSize: "0.8rem",
    },
  },
...
export default function Whitelabel(props) {
  ...
  async function handleSaveEnabledLanguages(newLangs) {
    console.log('🔍 Whitelabel - Salvando idiomas:', newLangs);
    await handleSaveSetting("enabledLanguages", JSON.stringify(newLangs));
    setEnabledLanguages(newLangs);
    
    // Limpar cache do sessionStorage para forçar recarregamento
    sessionStorage.removeItem('user_available_languages');
    sessionStorage.removeItem('language_feature_enabled');
    
    // Dispara eventos para atualizar o LanguageSelector
    localStorage.setItem('settingsUpdated', Date.now().toString());
    localStorage.setItem('enabledLanguages', JSON.stringify(newLangs));
    
    // Disparar múltiplos eventos para garantir que todos os componentes sejam atualizados
    window.dispatchEvent(new Event('storage'));
    window.dispatchEvent(new CustomEvent('languageSettingsUpdated', { detail: newLangs }));
    
    // Atualizar todos os frames/janelas abertas
    if (window.parent !== window) {
      window.parent.postMessage({ type: 'languageSettingsUpdated', languages: newLangs }, '*');
    }
  }

...
const UploaderCert = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!user.super) {
        toast.error(i18n.t("toasts.settings.noPermission"));
        setTimeout(() => {
          history.push(`/`)
        }, 500);
      }
...
const handleFileChange = (event) => {
  ...
  if (selectedFile) {
    ...
    if (fileName.endsWith(".p12")) {
      ...
      toast.error(i18n.t("toasts.settings.invalidFormat"));
    }
  }
};
  ...
  const handleSubmit = async (event) => {
    ...
    if (!file) {
      toast.warn(i18n.t("toasts.settings.fileRequired"));
      return;
    }

    if (!selectedOption) {
      toast.warn(i18n.t("toasts.settings.destinationRequired"));
      return;
    }
    ...
    try {
      ...
      if (response.data.mensagem === 'Arquivo Anexado') {
        setUploaded(true);
        toast.success(i18n.t("toasts.settings.fileSent"));
        ...
        <Grid item xs={12} sm={12} md={12} style={{ display: 'flex' }}>
  			<FormControl className={classes.fullWidth}>
      			...
      			<input
                    ...
                    style={{ marginTop: 15, marginBottom: 15 }}
      			/>
      			{selectedFileName ? selectedFileName : 'Escolher Certificado Efí p12 API PIX'}
    			</label>
  			</FormControl>
		</Grid>

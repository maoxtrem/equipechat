...
const PromptSchema = Yup.object().shape({
    name: Yup.string().min(5, i18n.t("promptModal.validation.tooShort")).max(100, i18n.t("promptModal.validation.tooLong")).required(i18n.t("promptModal.validation.required")),
    prompt: Yup.string().min(50, i18n.t("promptModal.validation.tooShort")).required(i18n.t("promptModal.validation.describeAI")),
    voice: Yup.string().required(i18n.t("promptModal.validation.informVoiceMode")),
    max_tokens: Yup.number().required(i18n.t("promptModal.validation.informMaxTokens")),
    temperature: Yup.number().required(i18n.t("promptModal.validation.informTemperature")),
    ...
    max_messages: Yup.number().required(i18n.t("promptModal.validation.informMaxMessages"))
});
...
const PromptModal = ({ open, onClose, promptId }) => {
    ...
    const handleSavePrompt = async values => {
        ...
        if (!values.queueId) {
            toastError(i18n.t("promptModal.validation.selectQueue"));
            return;
        }
        try {
            if (promptId) {
                                    ...
                                    <InputLabel>{i18n.t("promptModal.form.voice")}</InputLabel>
                                        ...
                                        >
                                            <MenuItem key={"texto"} value={"texto"}>
                                                {i18n.t("promptModal.voices.text")}
                                            </MenuItem>

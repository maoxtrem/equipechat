...
import { Edit as EditIcon, Schedule } from "@material-ui/icons";
            ...
            {(values) => (
                <Form className={classes.fullWidth}>
                    <Grid spacing={1} justifyContent="flex-start" container>
                        ...
                        {/* PUBLICO */}
                        <Grid xs={12} sm={6} md={1} xl={1} item>
                        ...
                        <Grid xs={12} sm={6} md={1} xl={1} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="trial-selection">Trial</InputLabel>
                                <Field
                                    as={Select}
                                    id="trial-selection"
                                    label="Trial"
                                    labelId="trial-selection-label"
                                    name="trial"
                                    margin="dense"
                                >
                                    <MenuItem value={true}>Sim</MenuItem>
                        ...
                        <Grid xs={12} sm={6} md={1} xl={1} item>
                            <Field
                                ...
                                type="number"
                                inputProps={{ min: 1 }}
                                disabled={!values.values.trial}
                            />
                        </Grid>

                        {/* NOME */}
                        <Grid xs={12} sm={6} md={3} xl={3} item>
                            <Field
                                as={TextField}
                        ...
                        <Grid xs={12} sm={6} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useWhatsappOfficial-selection">Whatsapp Oficial</InputLabel>
                                <Field
                                    as={Select}
                                    id="useWhatsappOfficial-selection"
                                    label="Whatsapp Oficial"
                                    labelId="useWhatsappOfficial-selection-label"
                                    name="useWhatsappOfficial"
                                    margin="dense"
                                >
                                    <MenuItem value={true}>{i18n.t("plans.form.enabled")}</MenuItem>
                                    <MenuItem value={false}>{i18n.t("plans.form.disabled")}</MenuItem>
...
export function PlansManagerGrid(props) {
    ...
    const renderWhatsappOfficial = (row) => {
        return row.useWhatsappOfficial === false ? `${i18n.t("plans.form.no")}` : `${i18n.t("plans.form.yes")}`;
    };
            ...
            >
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell align="center">{i18n.t("plans.form.trial")}</TableCell>
                ...
                <TableBody>
                    {records.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell align="center">{row.amount ? formatCurrency(row.amount) : formatCurrency(0)}</TableCell>
...
export default function PlansManager() {
    ...
    const { timezone, timezonesByRegion, setCurrentTimezone: updateTimezone } = useTimezone()
    
    const [showConfirmDialog, setShowConfirmDialog] = useState(false)
    const [selectedTimezone, setSelectedTimezone] = useState(timezone.value)
    
    // Verificar se deve mostrar configurações de fuso horário (apenas para admins normais)
    const showTimezoneSettings = user?.profile === 'admin' && !user?.super
    ...
    const handleSelect = (data) => {
        ...
        let useWhatsappOfficial = data.useWhatsappOfficial === false ? false : true
    ...
    const handleSaveTimezoneSettings = () => {
        updateTimezone(selectedTimezone);
        toast.success('Fuso horário do sistema salvo com sucesso!');
    };
    ...
    return (
        <>
            <Paper className={classes.mainPaper} elevation={0}>
                ...
                {/* Configuração de Fuso Horário - Apenas para admins normais */}
                    ...
                    <Box mb={2}>
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Grid item xs={12} sm={6} md={4}>
                                <FormControl margin="dense" variant="outlined" fullWidth>
                                    <InputLabel>Fuso Horário do Sistema</InputLabel>
                                    <Select
                                    ...
                                    >
                                        {Object.entries(timezonesByRegion).map(([region, timezones]) => [
                                            <MenuItem disabled key={region} style={{ backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>
                                                {region}
                                            </MenuItem>,
                                            ...timezones.map((tz) => (
                                                <MenuItem key={tz.value} value={tz.value} style={{ paddingLeft: 32 }}>
                                                    {tz.label}
                                                </MenuItem>
                                            ))
                                        ])}
                        ...
                        {/* Botão de salvar */}
                        <Grid container spacing={2} style={{ marginTop: 8 }} justifyContent="flex-start">
                        ...
                        <Box mt={1}>
                            <Typography variant="caption" color="textSecondary">
                                Esta configuração afeta a exibição de datas e horários em todo o sistema
                            </Typography>
                        </Box>
                    </Box>
                </Grid>
                ...
                <Grid xs={12} item>
                    <Typography variant="h6" style={{ marginBottom: 16 }}>Gerenciar Planos</Typography>
                    <PlanManagerForm
                        initialValue={record}

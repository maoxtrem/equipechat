...
import { useCurrency, CURRENCIES } from "../../utils/currencyUtils";
        ...
        {(values, setValues) => (
          <Form className={classes.fullWidth}>
            <Grid spacing={1} justifyContent="center" container>
              ...
              <Grid xs={12} sm={6} md={3} item>
                <FormControl margin="dense" variant="outlined" fullWidth>
                  <InputLabel htmlFor="currency-selection">
                    {i18n.t("compaies.table.currency")}
                  </InputLabel>
                  <Field
                    as={Select}
                    id="currency-selection"
                    label={i18n.t("compaies.table.currency")}
...
export function CompaniesManagerGrid(props) {
  ...
  const renderPlanValue = (row) => {
    return row.planId !== null && row.plan?.amount ? formatCurrency(row.plan.amount, row.currency) : "-";
  };
      ...
      >
        <TableHead>
          <TableRow>
            ...
            <TableCell align="center">{i18n.t("compaies.table.currency")}</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {records.map((row, key) => (
            <TableRow style={rowStyle(row)} key={key}>
              ...
              <TableCell style={cellStyle(row)} align="center">{renderPlanValue(row)}</TableCell>
              ...
              <TableCell style={cellStyle(row)} align="center">{row.currency || "BRL"}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>

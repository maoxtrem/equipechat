...
const SendWhatsAppMessage = async ({
  ...
  if (ENABLE_LID_DEBUG) {
    ...
    logger.info(
      `[LID-DEBUG] SendMessage - Contact remoteJid: ${contactNumber.remoteJid}`
    );
    logger.info(
  ...
  if (quotedMsg) {
    ...
    if (chatMessages) {
      ...
      if (ENABLE_LID_DEBUG) {
        logger.info(
          `[LID-DEBUG] SendMessage - ContextInfo configurado para resposta`
        );
      }
    }
  }
  ...
  try {
    ...
    if (err.message && err.message.includes("senderMessageKeys")) {
      if (ENABLE_LID_DEBUG) {
        logger.error(
          `[LID-DEBUG] SendMessage - Erro de criptografia de grupo detectado: ${err.message}`
        );
      }
      ...
      try {
        if (ENABLE_LID_DEBUG) {
          logger.info(
            `[LID-DEBUG] SendMessage - Tentando envio sem criptografia para grupo ${jid}`
          );
        }
        ...
        if (ENABLE_LID_DEBUG) {
          logger.info(
            `[LID-DEBUG] SendMessage - Sucesso no envio sem criptografia para grupo ${jid}`
          );
        }
        ...
        if (ENABLE_LID_DEBUG) {
          logger.error(
            `[LID-DEBUG] SendMessage - Falha no envio sem criptografia: ${finalErr.message}`
          );
        }

...
const sendMessage = async (
  ...
  if (ENABLE_LID_DEBUG) {
    ...
    logger.info(`[LID-DEBUG] ChatBot - Contact lid: ${contact.lid}`);
    logger.info(
      `[LID-DEBUG] ChatBot - Contact remoteJid: ${contact.remoteJid}`
    );
  }
  ...
  if (ticket.isGroup) {
    if (ENABLE_LID_DEBUG) {
      logger.info(`[LID-DEBUG] ChatBot - Enviando mensagem para grupo: ${jid}`);
    }
    ...
    try {
      ...
      if (ENABLE_LID_DEBUG) {
        logger.info(
          `[LID-DEBUG] ChatBot - Sucesso na primeira tentativa para grupo ${jid}`
        );
      }
    } catch (err1) {
      if (ENABLE_LID_DEBUG) {
        logger.warn(
          `[LID-DEBUG] ChatBot - Falha na primeira tentativa: ${err1.message}`
        );
      }
      ...
      if (err1.message && err1.message.includes("senderMessageKeys")) {
        if (ENABLE_LID_DEBUG) {
          logger.error(
            `[LID-DEBUG] ChatBot - Erro de criptografia de grupo detectado: ${err1.message}`
          );
        }
        ...
        try {
          if (ENABLE_LID_DEBUG) {
            logger.info(
              `[LID-DEBUG] ChatBot - Tentando envio sem criptografia para grupo ${jid}`
            );
          }
          ...
          if (ENABLE_LID_DEBUG) {
            logger.info(
              `[LID-DEBUG] ChatBot - Sucesso no envio sem criptografia para grupo ${jid}`
            );
          }
        } catch (finalErr) {
          if (ENABLE_LID_DEBUG) {
            logger.error(
              `[LID-DEBUG] ChatBot - Falha no envio sem criptografia: ${finalErr.message}`
            );
          }
          throw finalErr;
        }
        ...
        if (ENABLE_LID_DEBUG) {
          logger.info(
            `[LID-DEBUG] ChatBot - Sucesso na segunda tentativa para grupo ${jid}`
          );
        }
      }
    }

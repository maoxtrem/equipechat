...
# Função para normalizar nome da empresa para uso no PostgreSQL
normaliza_nome_empresa() {
  local nome="$1"
  # Converte para minúsculas
  nome=$(echo "${nome}" | tr '[:upper:]' '[:lower:]')
  # Remove acentos e caracteres especiais, substitui por underscore
  nome=$(echo "${nome}" | sed 's/[^a-z0-9]/_/g')
  # Remove underscores duplicados
  nome=$(echo "${nome}" | sed 's/__*/_/g')
  # Remove underscore do início e fim
  nome=$(echo "${nome}" | sed 's/^_//;s/_$//')
  # Se começar com número, adiciona prefixo
  case "${nome}" in
    [0-9]*) nome="app_${nome}" ;;
  esac
  # Se ficar vazio, usa nome padrão
...
carregar_variaveis() {
  if [ -f $ARQUIVO_VARIAVEIS ]; then
    ...
    # O nome já está normalizado quando foi salvo, não precisa normalizar novamente
  else
    empresa=$(normaliza_nome_empresa "Equipechat")
...
questoes_variaveis_base() {
  ...
  read -p "> " empresa
  echo
  # Normaliza nome da empresa para uso em banco/usuário usando a função
  empresa=$(normaliza_nome_empresa "${empresa}")
  printf "${GREEN} >> Nome normalizado para uso no sistema: ${empresa}${WHITE}\n"
  sleep 2
  # DEFINE SENHA BASE
  banner
  ...
  read -p "> " senha_deploy
  echo
  # Valida senha - remove caracteres problemáticos
  if [[ "${senha_deploy}" =~ [\'\"\$\\\`\|\;\&\<\>\(\)\{\}\[\]\*\?\#\!] ]]; then
    printf "${RED} >> Senha contém caracteres especiais não permitidos. Use apenas letras, números e _ - @ ${WHITE}\n"
    sleep 3
    questoes_variaveis_base
    return
  fi
  ...
  # DEFINE TOKEN GITHUB
  banner
  printf "${WHITE} >> Digite o token do GitHub (ou deixe vazio para instalação local): \n"
  echo
  read -p "> " github_token
  echo
  ...
  if [ -n "${github_token}" ]; then
    ...
    # Se não forneceu token, usa instalação local
    github_token="local_install"
    repo_url="local_install"
  fi
}
...
cria_banco_base() {
  ...
  echo
  {
    # Usa o nome já normalizado da variável empresa
    sudo su - postgres <<EOF
ROLE_EXISTS=$(psql -v ON_ERROR_STOP=1 -Atqc "SELECT 1 FROM pg_roles WHERE rolname='${empresa}'" || true)
if [ "\$ROLE_EXISTS" != "1" ]; then
  psql -v ON_ERROR_STOP=1 -c "CREATE ROLE ${empresa} LOGIN PASSWORD '${senha_deploy}';"
else
  psql -v ON_ERROR_STOP=1 -c "ALTER ROLE ${empresa} WITH LOGIN PASSWORD '${senha_deploy}';"
fi
DB_EXISTS=$(psql -v ON_ERROR_STOP=1 -Atqc "SELECT 1 FROM pg_database WHERE datname='${empresa}'" || true)
if [ "\$DB_EXISTS" != "1" ]; then
  psql -v ON_ERROR_STOP=1 -c "CREATE DATABASE ${empresa} OWNER ${empresa};"
fi
psql -v ON_ERROR_STOP=1 -d "${empresa}" -Atqc "ALTER DATABASE ${empresa} OWNER TO ${empresa};"
psql -v ON_ERROR_STOP=1 -d "${empresa}" -Atqc "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"
psql -v ON_ERROR_STOP=1 -d "${empresa}" -Atqc "CREATE EXTENSION IF NOT EXISTS \"unaccent\";"
EOF
...
# Clona código de repo privado ou copia local
baixa_codigo_base() {
  banner
  printf "${WHITE} >> Preparando o ${nome_titulo}...\n"
  echo
  {
    dest_dir="/home/deploy/${empresa}/"
    
    # Verifica se é instalação local
    if [ "${repo_url}" = "local_install" ] || [ "${github_token}" = "local_install" ]; then
      printf "${WHITE} >> Copiando código local...\n"
      echo
      
      # Define o diretório de origem (onde está o código fonte)
      source_dir="$(pwd)/codigo fonte/"
      
      # Verifica se o diretório existe
      if [ ! -d "${source_dir}" ]; then
        printf "${RED} >> Erro: Diretório 'codigo fonte/' não encontrado em ${source_dir}\n"
        printf "${WHITE} >> Por favor, certifique-se de executar o instalador do diretório correto.\n"
        exit 1
      fi
      ...
      # Copia outros diretórios se existirem
      [ -d "${source_dir}api_oficial" ] && cp -r "${source_dir}api_oficial" ${dest_dir}
      [ -d "${source_dir}api_transcricao" ] && cp -r "${source_dir}api_transcricao" ${dest_dir}
      
      printf "${WHITE} >> Código copiado com sucesso!\n"
      echo
    else
      # Instalação via GitHub (mantém o código original)
...
EOF
    ...
    sudo su - deploy <<EOF
  sed -i 's|npm3Binary = .*|npm3Binary = "/usr/bin/ffmpeg";|' /home/deploy/${empresa}/backend/node_modules/@ffmpeg-installer/ffmpeg/index.js
  mkdir -p /home/deploy/${empresa}/backend/node_modules/@ffmpeg-installer/linux-x64/ && \
  echo '{ "version": "1.1.0", "name": "@ffmpeg-installer/linux-x64" }' > /home/deploy/${empresa}/backend/node_modules/@ffmpeg-installer/linux-x64/package.json
EOF

    sleep 2

    banner
    printf "${WHITE} >> Verificando role/DB antes das migrações...\n"
    echo
    # Verifica se o banco e role existem (já foram criados anteriormente com nome normalizado)
    sudo su - postgres <<EOF
set -e
# Verifica se a role existe
ROLE_EXISTS=\$(psql -v ON_ERROR_STOP=1 -Atqc "SELECT 1 FROM pg_roles WHERE rolname='${empresa}'" || true)
if [ "\\\$ROLE_EXISTS" != "1" ]; then
  echo "ERRO: Role ${empresa} não existe. Criando..."
  psql -v ON_ERROR_STOP=1 -c "CREATE ROLE ${empresa} LOGIN PASSWORD '${senha_deploy}';"
fi
...
DB_EXISTS=\$(psql -v ON_ERROR_STOP=1 -Atqc "SELECT 1 FROM pg_database WHERE datname='${empresa}'" || true)
if [ "\\\$DB_EXISTS" != "1" ]; then
  echo "ERRO: Banco ${empresa} não existe. Criando..."
  psql -v ON_ERROR_STOP=1 -c "CREATE DATABASE ${empresa} OWNER ${empresa};"
  psql -v ON_ERROR_STOP=1 -d "${empresa}" -Atqc "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"
  psql -v ON_ERROR_STOP=1 -d "${empresa}" -Atqc "CREATE EXTENSION IF NOT EXISTS \"unaccent\";"
fi
...
PGPASSWORD="\${DB_PASS}" psql -h "\${DB_HOST}" -p "\${DB_PORT}" -U "\${DB_USER}" -d "\${DB_NAME}" -Atc "select 1" >/dev/null
timeout 60s ./node_modules/.bin/sequelize db:migrate:status
timeout 600s ./node_modules/.bin/sequelize db:migrate
EOF
...
set -e
cd /home/deploy/${empresa}/backend
timeout 600s ./node_modules/.bin/sequelize db:seed:all
EOF
...
instala_frontend_base() {
  ...
  # Limpa cache e node_modules para evitar conflitos
  rm -rf node_modules package-lock.json
  npm cache clean --force
  # Instala dependências com resolução forçada
  npm install --force --legacy-peer-deps
  npx browserslist@latest --update-db
  # Instala dependências necessárias para o override
  npm install -D react-app-rewired@2.2.1 \
    @babel/plugin-proposal-class-properties \
    @babel/plugin-proposal-private-methods \
    @babel/plugin-proposal-private-property-in-object \
    --legacy-peer-deps --force
  # Ajusta scripts para usar react-app-rewired
  sed -i "s/react-scripts start/react-app-rewired start/" package.json
  sed -i "s/react-scripts build/react-app-rewired build/" package.json
  sed -i "s/react-scripts test/react-app-rewired test/" package.json
  # Gera config-overrides.js compatível com CRA 5 (webpack 5)
  cat > /home/deploy/${empresa}/frontend/config-overrides.js << 'EOCONF'
...
EOSRV
    ...
    # Build com opções de memória e OpenSSL
...
backup_app_atualizar() {
  ...
  source /home/deploy/${empresa}/backend/.env
  {
    ...
    if [ "${confirmacao_backup}" == "S" ]; then
      ...
      db_user=$(grep "DB_USER=" /home/deploy/${empresa}/backend/.env | cut -d '=' -f2)
      db_name=$(grep "DB_NAME=" /home/deploy/${empresa}/backend/.env | cut -d '=' -f2)
      ...
      printf "${GREEN} >> Backup do banco de dados ${db_name} concluído. Arquivo de backup: ${backup_file}\n"
      sleep 2
    else
...
# Verifica se é instalação local ou GitHub
if [ "${repo_url}" = "local_install" ] || [ "${github_token}" = "local_install" ]; then
  printf "${WHITE} >> Modo de atualização local detectado, mantendo código existente...\\n"
else
  git reset --hard
  git pull
fi
...
sleep 2
cd /home/deploy/${empresa}/frontend
# Limpa completamente antes de atualizar
rm -rf node_modules package-lock.json
npm cache clean --force
npm install --force --legacy-peer-deps
sed -i 's/3000/'"$frontend_port"'/g' server.js
...
otimiza_banco_atualizar() {
  ...
  echo
  {
    ...
    sudo su - root <<EOF
    PGPASSWORD="$db_password" vacuumdb -U "${db_user}" -h localhost -d "${db_name}" --full --analyze
    PGPASSWORD="$db_password" psql -U ${db_user} -h 127.0.0.1 -d ${db_name} -c "REINDEX DATABASE ${db_name};"
    PGPASSWORD="$db_password" psql -U ${db_user} -h 127.0.0.1 -d ${db_name} -c "ANALYZE;"
EOF
